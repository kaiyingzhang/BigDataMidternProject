    ...
    public static class Reduce extends MapReduceBase implements Reducer<Text, Text, Text, Text> {  
	  ...
	  public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {  
		...
		sum = ((Text)values.next()).toString();  
		  output.collect(key,new Text(sum+","+num));
...
static	public final void crossOver(String cc1,String cc2) {
         ...
         // Mutation
	static	public final void mutate(String cc) {
		StringBuilder c = new StringBuilder(cc);
			for (int x=0;x<c.length();x++) {
				if (rand.nextDouble()<=mutRate) 
					c.setCharAt(x, (c.charAt(x)=='0' ? '1' : '0'));
			}
		}		  
    public static void main(String[] args) throws Exception {  
	  JobConf conf = new JobConf(MidtermExam1.class);  
      conf.setJobName("MidtermExam1");  
      ...
      conf.setOutputFormat(TextOutputFormat.class);  
	    ...
	    String output="output_midtermExam/output_midtermExam1";
      ...
      //FileOutputFormat.setOutputPath(conf, new Path(args[1]));  
		...
		String inputview="output_midtermExam/output_midtermExam1"+"\\part-00000";
		MidtermExam2 midtermExam2=new MidtermExam2(inputview);
    }
