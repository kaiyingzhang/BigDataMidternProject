 ...
 public class MidtermExam {  
    ...
    public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text,Text> {  
      ...
      public void map(LongWritable key, Text value, OutputCollector<Text,Text> output, Reporter reporter) throws IOException {  
		...
		for(Chomosone c : pool){
			
			String cB = c.decodeChromo();
			String cSc = String.valueOf(c.score);
			Text cScore = new Text(cSc);
			output.collect(cScore, new Text(cB)); 
    ...
    public static class Reduce extends MapReduceBase implements Reducer<Text, Text, Text, Text> {  
      public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {  
	   ...
	   while (values.hasNext()) { 
		...
		target = key.get();
		MidtermExam me = new MidtermExam();
		me.doIt(target);
		count++;
		if(result1 != null){
			  result = (new Text(result1).toString());
			  System.out.println("error1 ```` "+result);
			  }else{
			result = (new Text(result2).toString());
		...
		//int target =Integer.valueOf(result);
   ...
   /*String line = values.next().toString();  
	      MidtermExam me = new MidtermExam();
          int target = new IntWritable(values.next());  
		  me.doIt(target);
		  if(result1 != null){
			  output.collect(new Text(line.toString()), new Text(result1));
			  }else{output.collect(new Text(line.toString()), new Text(result2));}
		  */
  
    public static void main(String[] args) throws Exception {  
      JobConf conf = new JobConf(MidtermExam.class);  
      conf.setJobName("MidtermExam");  
      ...
      conf.setOutputFormat(TextOutputFormat.class);  
	    ...
	    String output="output_midtermExam";
	...
	private void doIt(int target) {
		...
		while(true) {
			...
			for(int x=pool.size()-1;x>=0;x-=2) {
				...
				//System.out.println("1n1:"+n1);
				//System.out.println("1n2:"+n2);
				...
				//System.out.println("2n1:"+n1.scoreChromo(target));
				//System.out.println("2n2:"+n2.scoreChromo(target));
				
				// Check to see if either is the solution
				if (n1.total == target && n1.isValid()) { 
				result1 = n1.decodeChromo();
				//System.out.println("result1:"+result1);
				System.out.println("Generations: " + gen + "  Solution: " + n1.decodeChromo()); return; }
				if (n2.total == target && n2.isValid()) { 
				result2 = n2.decodeChromo();
				//System.out.println("result2:"+result2);
				System.out.println("Generations: " + gen + "  Solution: " + n2.decodeChromo()); return; }
